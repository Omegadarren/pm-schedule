...
// ── Build flat rows: project-header rows interleaved with task rows ──────────

function buildMasterRows(tasks) {
  const projectOrder = [];
  const byProject = {};
  ...
  for (const pid of projectOrder) {
    ...
    const avgPct = taskList.length > 0
      ? Math.round(taskList.reduce((s, t) => s + (t.percent_complete || 0), 0) / taskList.length)
      : 0;
    ...
    rows.push({
      ...
      _projectColor: proj.color,
      name: proj.name,
      percent_complete: avgPct,
      cost: totalCost,
    });

    for (const t of taskList) {
      rows.push({ ...t, _projectColor: proj.color });
    }
  }
    ...
    return (
      <span style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 700, fontSize: 13, width: '100%' }}>
        <span style={{
          ...
          background: data._projectColor, flexShrink: 0, display: 'inline-block',
        }} />
...
const COLUMN_DEFS = [
  { field: 'wbs',              headerName: 'WBS',      width: 72,  resizable: true, cellRenderer: WbsCell },
  { field: 'name',             headerName: 'Task Name', flex: 1,   minWidth: 200, resizable: true, cellRenderer: NameCell },
  { field: 'project_name',     headerName: 'Project',   width: 160, resizable: true, cellRenderer: ProjectCell },
  { field: 'start_date',       headerName: 'Start',     width: 108, resizable: true, cellRenderer: DateCell },
  { field: 'end_date',         headerName: 'End',       width: 108, resizable: true, cellRenderer: DateCell },
  { field: 'duration_days',    headerName: 'Dur.',      width: 68,  resizable: true, cellRenderer: DurationCell },
  { field: 'status',           headerName: 'Status',   width: 118, resizable: true, cellRenderer: StatusCell },
  { field: 'priority',         headerName: 'Priority',  width: 88,  resizable: true, cellRenderer: PriorityCell },
  { field: 'percent_complete', headerName: '% Done',    width: 136, resizable: true, cellRenderer: ProgressCell },
  { field: 'cost',             headerName: 'Cost',      width: 108, resizable: true, cellRenderer: CostCell },
];

const DEFAULT_COL_DEF = { resizable: true, sortable: false };

export default function MasterGrid({ tasks }) {
  const rows = useMemo(() => buildMasterRows(tasks), [tasks]);

  const getRowStyle = (params) => {
    if (params.data?._isProjectHeader) {
      return {
        background: `${params.data._projectColor}1a`,
        borderLeft: `3px solid ${params.data._projectColor}`,
      };
    }
    return null;
  };

  const getRowHeight = (params) => (params.data?._isProjectHeader ? 42 : 36);
